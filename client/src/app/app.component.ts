...
import {AccountService, AuthenticationService, Profile, Role, SetupService} from './api';
import {SubscriptionsPageComponent} from './pages/subscriptions-page.component';
...
import {AdminJellyfinPageComponent} from './pages/admin-jellyfin-page.component';
import {AdminCachePageComponent} from './pages/admin-cache-page.component';
import {AdminJobsPageComponent} from './pages/admin-jobs-page.component';
import {AuthPageComponent} from './pages/auth-page.component';
import {InitialSetupPageComponent} from './pages/initial-setup-page.component';
...
interface Video {
    ...
    cache_status?: 'NOT_CACHED' | 'PARTIAL' | 'COMPLETE';
...
@Component({
    selector: 'app-root',
    imports: [CommonModule, FormsModule, SubscriptionsPageComponent, VideosPageComponent, DownloadsPageComponent, AccountPageComponent, AdminUsersPageComponent, AdminSettingsPageComponent, AdminJellyfinPageComponent, AdminCachePageComponent, AdminJobsPageComponent, AuthPageComponent, InitialSetupPageComponent],
...
export class AppComponent implements OnInit {
    ...
    readonly me = signal<ProfileValue | null>(null);
    ...
    auth(): void {
        ...
        if (this.authMode() === 'register' && this.password.length < 12) {
        ...
        this.api.post(url, {
            ...
            error: (e: unknown) => {
                ...
                this.error.set(this.messageFor(e, 'Unable to authenticate.'));
            }
        });
    }

    load(): void {
        this.api.get<Profile>('/api/me').subscribe({
            next: profile => {
                this.me.set(profile);
                this.loading.set(false);
                this.clearMessages();
                this.loadUserData();
                if (profile.role === 'ADMIN') this.loadAdminData();
            },
            error: () => {
                this.me.set(null);
                this.loading.set(false);
                ...
                }>('/api/setup/status').subscribe(value => this.setupRequired.set(value.setupRequired));
            }
        });
    }
    ...
    private loadUserData(): void {
        this.api.get<Subscription[]>('/api/subscriptions').subscribe({
            ...
            error: e => this.error.set(this.messageFor(e, 'Could not load subscriptions.'))
        });
        this.api.get<Video[]>('/api/videos').subscribe({
            next: rows => this.videos.set(rows),
            error: e => this.error.set(this.messageFor(e, 'Could not load videos.'))
        });
    }

    private loadAdminData(): void {
        this.api.get<Record<string, string>>('/api/admin/settings').subscribe({
            next: value => this.settings.set(value),
            error: e => this.error.set(this.messageFor(e, 'Could not load settings.'))
        });
        this.api.get<AdminUser[]>('/api/admin/users').subscribe({
            next: value => this.adminUsers.set(value),
            error: e => this.error.set(this.messageFor(e, 'Could not load users.'))
        });
        this.api.get<CacheEntry[]>('/api/admin/cache').subscribe({
            next: value => this.cacheEntries.set(value),
            error: e => this.error.set(this.messageFor(e, 'Could not load cache status.'))
        });
        this.api.get<Job[]>('/api/admin/jobs').subscribe({
            next: value => this.jobs.set(value),
            error: e => this.error.set(this.messageFor(e, 'Could not load jobs.'))
        });
    ...
    private loadJellyfinStatus(): void {
        ...
        this.api.get<JellyfinStatus>('/api/admin/jellyfin/status').subscribe({
    ...
    validateJellyfin(): void {
        ...
        this.api.post<JellyfinStatus>('/api/admin/jellyfin/validate', {}).subscribe({
            next: value => {
                this.jellyfinStatus.set(value);
                this.saving.set(false);
                this.notice.set(value.reachable ? 'Jellyfin connection validated.' : `Jellyfin validation failed: ${value.message || 'server unreachable'}`);
            },
            error: e => {
                this.saving.set(false);
                this.error.set(this.messageFor(e, 'Could not validate Jellyfin connection.'));
            }
        });
    }
    ...
    refreshJellyfin(): void {
        this.saving.set(true);
        this.api.post<{ success?: boolean; message?: string }>('/api/admin/jellyfin/refresh', {}).subscribe({
            next: result => {
                this.saving.set(false);
            ...
            error: e => {
                this.saving.set(false);
                this.error.set(this.messageFor(e, 'Could not request Jellyfin library refresh.'));
            }
        });
    }
    ...
    addSubscription(): void {
        ...
        this.loading.set(true);
        this.api.post('/api/subscriptions', {channel: value}).subscribe({
            next: () => {
                this.channel = '';
            ...
            error: (e: unknown) => {
                this.loading.set(false);
                this.error.set(this.messageFor(e, 'Could not add subscription.'));
            }
        });
    }
    ...
    toggleSubscription(subscription: Subscription): void {
        ...
        this.api.patch(`/api/subscriptions/${subscription.id}`, {enabled}).subscribe({
            next: () => {
    ...
    refreshSubscription(subscription: Subscription): void {
        this.loading.set(true);
        this.api.post<{ discovered?: number }>(`/api/subscriptions/${subscription.id}/refresh`, {}).subscribe({
            next: result => {
                this.loading.set(false);
            ...
            error: (e: unknown) => {
                this.loading.set(false);
                this.error.set(this.messageFor(e, 'Could not refresh subscription.'));
            }
        });
    }
    ...
    removeSubscription(subscription: Subscription): void {
        ...
        this.api.delete(`/api/subscriptions/${subscription.id}`).subscribe({
            next: () => {
    ...
    changePassword(): void {
        ...
        this.saving.set(true);
        this.api.post('/api/me/change-password', {
            ...
            error: (e: unknown) => {
                this.saving.set(false);
                this.error.set(this.messageFor(e, 'Could not change password.'));
            }
        });
    }
    ...
    saveSettings(): void {
        this.saving.set(true);
        this.api.patch('/api/admin/settings', this.settings()).subscribe({
            next: () => {
                this.saving.set(false);
            ...
            error: (e: unknown) => {
                this.saving.set(false);
                this.error.set(this.messageFor(e, 'Could not save settings.'));
            }
        });
    }

    updateRole(user: AdminUser, role: Role): void {
        if (user.id === this.me()?.id && role !== 'ADMIN') {
            this.error.set('You cannot remove your own administrator access.');
            return;
        }
        this.api.patch(`/api/admin/users/${user.id}`, {role}).subscribe({
            next: () => {
    ...
    deleteCache(entry: CacheEntry): void {
        ...
        this.api.delete(`/api/admin/cache/${encodeURIComponent(entry.video_id)}`).subscribe({
            next: () => {
    ...
    cancelJob(job: Job): void {
        this.api.post(`/api/admin/jobs/${encodeURIComponent(job.id)}/cancel`, {}).subscribe({
            next: () => {
    ...
    enabled(subscription: Subscription): boolean {
        return subscription.enabled === true || subscription.enabled === 1;
    }
    ...
    createInitialAdmin(): void {
        ...
        if (this.password.length < 12 || !this.setupToken.trim()) {
            this.error.set('A setup token and a password of at least 12 characters are required.');
            return;
        }
        this.loading.set(true);
        this.api.post('/api/setup/create-admin', {
            username: this.username.trim(),
            email: this.email.trim() || null,
            password: this.password,
            token: this.setupToken.trim()
        }).subscribe({
            next: () => {
                this.setupToken = '';
                this.password = '';
                ...
                this.loading.set(false);
                this.error.set(this.messageFor(e, 'Could not create the administrator.'));
            }
        });
    }
    ...
    formatDuration(seconds: number | null | undefined): string {
        if (!seconds || seconds < 1) return 'Runtime pending';
        const hours = Math.floor(seconds / 3600);
        const minutes = Math.floor((seconds % 3600) / 60);
        const remainder = seconds % 60;
        return hours > 0 ? `${hours}:${this.twoDigits(minutes)}:${this.twoDigits(remainder)}` : `${minutes}:${this.twoDigits(remainder)}`;
    }
